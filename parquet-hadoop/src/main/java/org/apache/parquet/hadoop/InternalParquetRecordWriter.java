...
public class InternalParquetRecordWriter<T> {
  ...
  public void write(T value, int numberOfValuesExpected) throws IOException, InterruptedException {
    checkBlockSizeReached(numberOfValuesExpected); //if there is no room for the given number of rows in the current row group - flush row group and create new one
    writeSupport.write(value);
    ++ recordCount;
  }
  ...
  private void checkBlockSizeReached(int numberOfRecords) throws IOException {
    if (recordCount > 0/*= recordCountForNextMemCheck*/) {
      long memSize = columnStore.getBufferedSize();
      long recordSize = memSize / recordCount;
      // flush the row group if it is within ~2 records of the limit
      // it is much better to be slightly under size than to be over at all
      if (memSize > (nextRowGroupSize - (numberOfRecords + 2) * recordSize)) {
